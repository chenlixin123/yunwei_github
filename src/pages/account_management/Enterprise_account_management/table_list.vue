    ...
    >
      ...
      <div style="padding-left: 5px">
        ...
        <div class="module_divs">
          ...
          <Input
            v-if="module_title == '新增企业账号'"
            v-model="add_data.account"
        ...
        <div class="module_divs">
          ...
          <Input
            v-if="module_title == '新增企业账号' || input_true == true"
...
import {
  //账号列表
  admin_enterprise_list,
  //新增账号
  admin_enterprise_save,
  //编辑账号
  admin_enterprise_update,
  //账号删除
  admin_enterprise_delete,
  //启用-禁用
  admin_enterprise_update_flag,
...
export default {
            ...
            menu = h("div", [
              h("i-switch", {
                props: {
                  type: "warning",
                  size: "large",
                  value: params.row.enterpriseFlag === 1, // 控制开关的打开或关闭状态，官网文档属性是value
                },
                style: {
                  marginRight: "10px",
                },
                ...
                on: {
                  "on-change": (state) => {
                    ...
                    if (state) {
                      this.change_enterpriseFlag_open(id);
                    } else {
                      this.change_enterpriseFlag_close(id);
                    }
                  },
                },
              }),
  ...
  methods: {
    ...
    modalok() {
      ...
      if (!this.add_data.account) {
        this.$Message.error("请填写账号名");
        this.loading_btn = false;
        return;
      }
      if (this.module_title == "新增企业账号") {
      ...
      if (this.module_title == "新增企业账号") {
        this.add_data.enterpriseFlag = 1;
        admin_enterprise_save(this.add_data).then((res) => {
          //(res);
          if (res.success == true) {
        ...
        let data = {
          preId: this.add_data.preId,
          account: this.add_data.account,
          password: this.add_data.password,
          address: this.add_data.address,
          name: this.add_data.name,
          tellphone: this.add_data.tellphone,
          analysisFlag: this.add_data.analysisFlag,
          enterpriseFlag: this.add_data.enterpriseFlag,
          type: 2,
          password: this.add_data.password,
          id: this.edit_id,
        };
        //(data);
        admin_enterprise_update(data).then((res) => {
          //(res);
          if (res.success == true) {
            this.$Message.success(res.content);
            this.modal9 = false;
    ...
    delete(e) {
      ...
      this.$Modal.confirm({
        title: "确定删除",
        onOk: () => {
          admin_enterprise_delete({ id: e.id }).then((res) => {
            //(res);
            if (res.success == true) {
              this.$Message.success(res.content);
              this.delete_getData();
    ...
    edit(val) {
      ...
      this.add_data = {
        ...
        analysisFlag: val.analysisFlag,
        enterpriseFlag: val.enterpriseFlag,
        type: 2,
        password: "",
      };
      this.edit_id = val.id;
    ...
    //禁用
    change_enterpriseFlag_close(id) {
      admin_enterprise_update_flag({
        id: id,
        type: 0,
      }).then((res) => {
