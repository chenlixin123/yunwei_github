            ...
            return new Promise((resolve, reject) => {
                ...
                AccountLogin({
                    ...
                    .then(async res => {
                        ...
                        if (res.login == true) {
                            util.cookies.set('uuid', res.map.id);
                            util.cookies.set('token', res.token);
                            res.map.analysisFlag == 1 ? res.map.access = ['admin'] : res.map.access = ['no'] // console.log(dispatch)
                            // 设置 vuex 用户信息
                            await dispatch('admin/user/set', res.map, {
                                root: true
                            });
                            // 用户登录后从持久化数据加载一系列的设置
                            await dispatch('load');
